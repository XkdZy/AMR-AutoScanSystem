...
def main(image_path, category_name_string, params, session, model):
    ...
    print("segmentations:", segmentations[indices_fg].shape)
    print("___________________________________________________________________________")
    ...
    # print(image_path)
    dir_path = './imgs/rgbd_frame/vild_result/{}/'.format(image_path.split('\\')[-1].split('.')[0])
    ...
    for anno_idx in indices[0:int(n_boxes)]:
        ...
        # cv2.namedWindow("raw_image", 0)
        # cv2.resizeWindow("raw_image", 640, 480)
        # cv2.imshow("raw_image", raw_image)
        # cv2.rectangle(raw_image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 5)
        # cv2.namedWindow("raw_image_Rect", 0)
        # cv2.resizeWindow("raw_image_Rect", 640, 480)
        # cv2.imshow("raw_image_Rect", raw_image)
        # cv2.waitKey(0)
...
def VildInference():
    ...
    print("inter VildInference...")
    # tf.device('/gpu:0')
    tf.device('/cpu:0')
    print("tensorflow import suceess...")
    session = tf.Session(graph=tf.Graph())
    saved_model_dir = './imgs/image_path_v2'  # @param {type:"string"}
    print("load model...")
    _ = tf.saved_model.loader.load(session, ['serve'], saved_model_dir)
    print("load model successful...")
    # if __name__ == "__main__":
    for filename in glob.glob('./imgs/rgbd_frame/rgb/*.jpg'):
        print("inter file successful...")
        print(filename)
        # print(filename[12:-4])
        image_path = filename
        # image_path = './1.bmp'  #@param {type:"string"}
        # image_path = './0002.png'  #@param {type:"string"}
        # image_path = './arc1.jpg'  #@param {type:"string"}
        # image_path = './Projection.jpg'  #@param {type:"string"}
        #display_image(image_path, size=display_input_size)
        # category_name_string = ';'.join(['ship-like'])
        # category_name_string = ';'.join(['curve piece'])
        # category_name_string = ';'.join(['checkboard'])
        # category_name_string = ';'.join(['test piece'])
        # category_name_string = ';'.join(['flat box'])
        category_name_string = ';'.join(['box'])
        max_boxes_to_draw = 16 #@param {type:"integer"}

        nms_threshold = 0.9 #@param {type:"slider", min:0, max:0.9, step:0.05}
        min_rpn_score_thresh = 0.5  #@param {type:"slider", min:0, max:1, step:0.01}
        min_box_area = 1000 #@param {type:"slider", min:0, max:10000, step:1.0}
